...
const images: ImageItem[] = [
  {
    id: '1',
    text: 'Find Your Buddy and Stay Connected Together!',
...
const FriendshipNavigator: React.FC = () => {
  const navigation = useNavigation<StackNavigationProp<any>>();
  const [currentIndex, setCurrentIndex] = useState<number>(0);
  const flatListRef = useRef<FlatList<ImageItem>>(null);
  const windowWidth = Dimensions.get('window').width;
  const containerWidth = windowWidth; // Use the full window width
  ...
  const renderItem = ({ item }: { item: ImageItem }) => (
    <View style={[styles.itemContainer, { width: 340 , height: 150}]}>
...
export default FriendshipNavigator;
